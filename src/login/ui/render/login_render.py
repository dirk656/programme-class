...
from config.settings import COLOR_PRIMARY, COLOR_BACKGROUND
...
class LoginRender:
    ...
    def build(self):
        """构建整个登录界面：布局、控件、样式"""
        self.win.setStyleSheet(get_global_style())

        # 中心圆角部件
        central_widget = QWidget()
        central_widget.setStyleSheet(f"background-color: {COLOR_BACKGROUND}; border-radius: 10px;")
        ...
        divider.setStyleSheet(f"background-color: {COLOR_PRIMARY};")
        ...
        # 标题
        title = QLabel("用户登录")
        title.setStyleSheet(f"font-size:28px; font-weight:bold; color:{COLOR_PRIMARY};")
